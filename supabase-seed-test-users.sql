  ...
  for member in select * from jsonb_array_elements(crew) loop
    ...
    update public.profiles
       set name = member->>'name',
           ...
           -- Yacht-specific fields so the deck showcases marina/season matching.
           home_port = (array['Antibes (Port Vauban)','Palma de Mallorca','Monaco (Port Hercule)','Fort Lauderdale','Antigua (English Harbour)','Genoa'])[1 + floor(random()*6)::int],
           season = (array['Mediterranean season','Caribbean season','Shipyard / Refit','Crossing / Delivery'])[1 + floor(random()*4)::int],
           availability = (array['Available','On charter','On rotation / leave','Between seasons'])[1 + floor(random()*4)::int],
           verified = (random() < 0.6),
           prompts = jsonb_build_array(
             jsonb_build_object('prompt','Med or Caribbean?','answer',(array['Med all the way','Caribbean for the winter','Both - I follow the sun'])[1 + floor(random()*3)::int]),
             jsonb_build_object('prompt','Charter or private?','answer',(array['Charter - love the buzz','Private, calmer life','Done both'])[1 + floor(random()*3)::int])
           )
